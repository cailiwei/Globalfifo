...
#define MAX_CMDS_CNT 				512
static struct param_dev fifo_param[MAX_CMDS_CNT];
static struct param_cmds_info *fifo_cmds = NULL;
#endif
...
#ifdef CONFIG_GLOBALFIFO_ENABLE
static void get_panel_info_pt(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int i = 0, temp_cnt = 0;
	struct dsi_panel_cmds *panel_cmds = &(ctrl_pdata->on_cmds);
	int cnt = ctrl_pdata->on_cmds.cmd_cnt;	

	fifo_cmds = kmalloc(sizeof(struct param_cmds_info), GFP_KERNEL);
	if (!fifo_cmds) {
		printk(KERN_ALERT "%s: Failed to alloc fifo_cmds.\n", __func__);
	}
	memset(fifo_cmds, 0, sizeof(struct param_cmds_info));

	if (cnt == panel_cmds->cmd_cnt) {
		for (i = 0; i < cnt; i++) {

			fifo_param[i].size = panel_cmds->cmds[i].dchdr.dlen;
			fifo_param[i].param = panel_cmds->cmds[i].payload;	
			temp_cnt += fifo_param[i].size;

#ifdef FIFO_CDEV_DEBUG
			printk("fifo_debug: fifo_param[%02d].size = %02d, \
								fifo_param[%02d].param[0] = 0x%02x \n",
								i, fifo_param[i].size,
								i, fifo_param[i].param[0]);
#endif
		}
	}

	if ((cnt + temp_cnt + 1) < GLOBALFIFO_SIZE) {
		fifo_cmds->cnt = cnt;
		fifo_cmds->cmds = fifo_param;
	} else {
		printk("clw: the data too large to save fifo_cdev\n");
	}
}
#endif
...
#ifdef CONFIG_GLOBALFIFO_ENABLE
		get_param_from_cdev(fifo_cdevp, fifo_cmds);
#endif
...
#ifdef CONFIG_GLOBALFIFO_ENABLE			
	    put_param_into_cdev(fifo_cmds, fifo_cdevp);
#endif
